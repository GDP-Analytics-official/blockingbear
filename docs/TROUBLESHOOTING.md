...
| A chat turn ends with `No endpoints found matching your data policy` | The model has no endpoint with Zero Data Retention, so the server-side privacy rules leave OpenRouter nothing to route to | Choose a model whose selector lists ZDR providers, or let an administrator enable the non-ZDR exception in **Settings → Chat LLM** and turn it on for that conversation |
| A chat turn ends with `No endpoints found that can handle the requested parameters` | An endpoint of that model rejects a request parameter under strict routing | The backend retries the request automatically with softer routing and logs `OpenRouter routing refused`; if the message still reaches the chat, check the backend log for that line and report the model id |
